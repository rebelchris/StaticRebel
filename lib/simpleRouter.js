...
const QUERY_PATTERNS = [
  /^how (many|much)/i,
  /^show (me )?(my )?/i,
  /^what('s| is| are| did) (my )?/i,
  /^display/i,
  /\bstats\b/i,
  /^today'?s? /i,  // "today's calories" at START is query
  /\bhistory\b/i,
  /\btotal\b/i,
  /\bprogress\b/i,
  /did i (have|eat|drink|do)/i,
];
...
const LOG_PATTERNS = [
  ...
  /\bdrank\b/i,
  /\bdid\b.*\d+/i,
  /\b(lunch|dinner|breakfast)\b.*\d+/i,
  /\d+.*(lunch|dinner|breakfast)/i,
];

function detectIntent(input) {
  const lower = input.toLowerCase();
  
  // Check for LOG intent FIRST (more specific)
  // If it has a number + unit, it's probably a log
