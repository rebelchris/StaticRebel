...
export async function routeSimply(input, options = {}) {
  ...
  return await handleConversation(input, startTime, context);
}
...
async function handleSkillLog(skill, extracted, skillManager, startTime) {
  try {
    // Ensure skill data file exists with proper structure
...
async function handleConversation(input, startTime, context = {}) {
  try {
    ...
    if (memoryContext) {
      // Prepend memory context as system context
      enrichedInput = `[Context from memory]\n${memoryContext}\n\n[User message]\n${input}`;
    }

    const response = await sendMessage(enrichedInput);
    return {
      success: true,
      type: 'chat',
