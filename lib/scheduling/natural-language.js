...
export function parseNaturalLanguage(input) {
  ...
  const parsers = [
    parseInterval,
    parseWeeklyTime,      // Check weekly patterns before daily patterns
    parseSpecificDateTime,
    parseRelativeTime,
    parseDailyTime,       // Daily patterns come after weekly ones
    parseEveryPattern,
    parseAtPattern
  ];
...
function parseWeeklyTime(input) {
  // Match various weekly patterns
  const weeklyPatterns = [
    // Basic patterns like "every Monday at 10am"
    ...
    // Patterns like "weekly on Friday at 3pm"
