...
// ============================================================================
// Conversation Checkpoints Integration
// ============================================================================

/**
 * Save conversation checkpoint
 */
export async function saveConversationCheckpoint(name, description = '') {
  const sessionMemory = getSessionMemory();
  const checkpointId = await saveCheckpoint(name, sessionMemory, description);
  return { 
    success: true, 
    checkpointId, 
    message: `Checkpoint '${name}' saved successfully.`,
 ...
 */
export async function restoreConversationCheckpoint(checkpointId) {
  const sessionMemory = getSessionMemory();
  const result = await restoreCheckpoint(checkpointId, sessionMemory);
  return {
    success: true,
    message: `Restored to checkpoint '${result.checkpoint.name}'.`,
...
export async function listConversationCheckpoints() {
  const sessionMemory = getSessionMemory();
  const checkpoints = await listCheckpoints(sessionMemory.sessionId || 'default');
  return {
    success: true,
    checkpoints,
...
/**
 * Create auto checkpoint before major operations
 */
export async function createAutoCheckpoint(action) {
  const sessionMemory = getSessionMemory();
  const checkpointId = await autoCheckpoint(action, sessionMemory);
  
  if (checkpointId) {
    return {
      success: true,
      checkpointId,
      message: `Auto checkpoint created before ${action}.`
    };
  }
  
  return {
    success: false,
    message: `Auto checkpoints disabled or action '${action}' not configured for auto-checkpoint.`
  };
}
