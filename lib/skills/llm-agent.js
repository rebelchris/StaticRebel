...
- When unsure, ask for clarification naturally
- Celebrate streaks and goal completions!
- If no matching skill exists, offer to create one
  ...
  async processMessage(chatId, message) {
    ...
    if (response.toolCalls && response.toolCalls.length > 0) {
      ...
      if (toolResults.length > 0) {
        ...
        this.addToHistory(chatId, 'assistant', finalResponse.content);
    ...
    this.addToHistory(chatId, 'assistant', response.content);
