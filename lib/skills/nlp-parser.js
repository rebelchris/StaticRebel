...
const QUERY_PATTERNS = [
  // Skill-specific queries - must reference tracking/progress concepts
  /^how (much|many) .*(water|coffee|steps|push ?ups?|exercise|sleep|meditation|reading)/i,
  /^how('s| is| are| was) my /i,  // "how's my water?" but not "how big is the moon?"
  /^what('s| is) my /i,  // "what's my progress?" not "what is the capital?"
  /^show (me |my )/i,
  /^check (my )/i,
  /^(did|have) I (track|log|do|drink|eat|walk|run|sleep|exercise)/i,
  /^my .+ (today|this week|yesterday)/i,
  /\b(status|progress|total|summary|stats)\b.*\?$/i,  // only if ends with ? AND has tracking word
  /^tell me (about )?my/i,
  // Skill name + question mark (e.g., "water?" "pushups?")
  /^(water|coffee|steps|push ?ups?|squats?|exercise|sleep|mood|meditation|reading)\s*\??$/i,
];
...
const SKILL_ALIASES = {
  ...
  // Expenses and money
  expenses: ['spent', 'expense', 'cost', 'paid', 'money', 'dollar', 'bought', 'purchase'],
};
...
export const SKILL_DEFAULTS = {
  ...
  expenses: { unit: 'USD', goal: null, icon: '💰', name: 'Expenses' },
};
