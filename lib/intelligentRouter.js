...
### For use_skill:
- ONLY match if the TOPIC of the input matches the skill's PURPOSE
- The skill must be SEMANTICALLY CORRECT for what the user is tracking

**STRICT MATCHING:**
- "water" skill = ONLY for water/hydration (glasses of water, ml of water, bottles of water)
- "water" skill is NOT for: food, calories, lunch, dinner, meals, kcal
- "coffee" skill = ONLY for coffee drinks
- "steps" skill = ONLY for walking/steps
- "pushups" skill = ONLY for pushups exercise

**WRONG matches (NEVER do this):**
- "400kcal lunch" → water skill ❌ (lunch is FOOD, not water!)
- "ate pizza" → water skill ❌ (food is not water!)
- "had a meal" → water skill ❌ (meals are not water!)

**CORRECT matches:**
- "drank 500ml water" → water skill ✅
- "2 glasses of water" → water skill ✅ (value: 500ml)
- "had a coffee" → coffee skill ✅
- "walked 5000 steps" → steps skill ✅

**Value extraction:**
- "500ml water" → value: 500, unit: "ml"
- "2 glasses of water" → value: 500, unit: "ml" (1 glass ≈ 250ml)
- "2L water" → value: 2000, unit: "ml"

### For create_skill:
- ONLY use when LOGGING data for something with NO matching skill
- "400kcal lunch" → create_skill (no food/calories skill exists!)
- "ate 500 calories" → create_skill (no calories skill exists!)
- "ran 5km" → create_skill (if no running skill exists)
- Propose a sensible skill with appropriate unit
- NEVER create_skill for QUERIES like "how many X today?"

### QUERIES vs LOGS:
- "My lunch was 400kcal" = LOG (wants to record data) → skillAction: "log"
- "How many calories today?" = QUERY (wants to see data) → skillAction: "query"
- "Show my water stats" = QUERY → skillAction: "stats"
- If querying a skill that doesn't exist, respond with chat explaining no data exists
...
async function proposeSkillCreation(decision, input, context) {
  ...
  const skillManager = await getSkillManager();
  await skillManager.init(); // Ensure skills are loaded
  
  // Check if a similar skill already exists
  const proposedId = proposedSkill?.name?.toLowerCase().replace(/[^a-z0-9_]/g, '_') || '';
  const existingSkill = skillManager.skills.get(proposedId) || 
    Array.from(skillManager.skills.values()).find(s => 
      s.name.toLowerCase() === proposedSkill?.name?.toLowerCase() ||
      s.triggers?.some(t => proposedSkill?.name?.toLowerCase().includes(t))
    );
  
  if (existingSkill) {
    // Skill already exists! Use it instead of creating
    if (CONFIG.DEBUG) {
      console.log(`[IntelligentRouter] Skill "${existingSkill.id}" already exists, using it`);
    }
    
    // Re-route to use_skill
    return await executeSkill({
      skillId: existingSkill.id,
      skillAction: decision.skillAction || 'log',
...
async function executeSkillCreation(proposedSkill, originalInput, skillManager) {
  try {
    // Use SkillManager to create the skill
    const skillDef = {
      description: proposedSkill.description || `Track ${proposedSkill.name}`,
      unit: proposedSkill.unit || '',
      type: proposedSkill.type || 'number',
      triggers: proposedSkill.triggers || [proposedSkill.name.toLowerCase()],
      dailyGoal: proposedSkill.dailyGoal || null,
      icon: proposedSkill.icon || '📊',
    };
    
    // Sanitize skill name for use as ID
    const skillId = proposedSkill.name.toLowerCase().replace(/[^a-z0-9_]/g, '_');
    
    const result = await skillManager.createSkill(skillId, skillDef);
    
    if (result) {
      // Reload skills to include the new one
      await skillManager.loadAllSkills();
      
      let response = `✨ Created new skill: **${proposedSkill.name}**\n${skillDef.description}\n\nYou can now track this with phrases like: "${skillId} [value]"`;
    ...
    return {
      success: false,
      type: 'skill_creation_failed',
      content: `Failed to create skill: Unknown error`,
    };
  } catch (error) {
