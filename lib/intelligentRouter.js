...
// Simple in-memory state for pending confirmations
// In production, this should be persisted per-user/session
const pendingConfirmations = new Map();

function setPendingConfirmation(sessionId, data) {
  pendingConfirmations.set(sessionId || 'default', {
    ...data,
    timestamp: Date.now(),
  });
}
...
function getPendingConfirmation(sessionId) {
  const pending = pendingConfirmations.get(sessionId || 'default');
  // Expire after 5 minutes
  if (pending && Date.now() - pending.timestamp < 5 * 60 * 1000) {
    return pending;
  }
  pendingConfirmations.delete(sessionId || 'default');
  return null;
}
