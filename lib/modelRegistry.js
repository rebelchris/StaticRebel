    ...
    const req = http.request({
      ...
      res.on('end', () => {
        try {
          ...
          resolve(new OllamaHealthStatus(true, OLLAMA_HOST, null, parsed.models || []));
        } catch (e) {
          resolve(new OllamaHealthStatus(false, OLLAMA_HOST, 'Invalid response from Ollama'));
        }
      });
    });

    req.on('error', (err) => {
      resolve(new OllamaHealthStatus(false, OLLAMA_HOST, err.code || err.message));
    });
    req.on('timeout', () => {
      req.destroy();
...
export async function listAvailableModels() {
  ...
  if (!health.connected) {
    ...
    availableModelsCache = { error: health.error, host: health.host };
    cacheExpiry = Date.now() + CACHE_DURATION;
    return [];
  }

  availableModelsCache = health.models;
  cacheExpiry = Date.now() + CACHE_DURATION;
