...
const MASTER_KEY_ENV = 'STATIC_REBEL_MASTER_KEY';
...
function getEncryptionKey() {
  ...
  if (envKey) {
    ...
    return scryptSync(envKey, 'static-rebel-salt', 32);
  }
  // Generate a deterministic key from machine-specific data
  // Note: In production, require the user to set MASTER_KEY_ENV
  const machineId = `${os.hostname()}-${os.userInfo().username}`;
  return scryptSync(machineId, 'static-rebel-salt', 32);
}
...
export function loadApiKeys() {
  try {
    ...
    if (!data.encrypted) {
      ...
      const encryptedData = { keys: encryptedKeys, encrypted: true };
      fs.writeFileSync(API_KEYS_FILE, JSON.stringify(encryptedData, null, 2));
    ...
    for (const [key, value] of Object.entries(data.keys || {})) {
      try {
        ...
        console.error(
          `[ApiConnector] Failed to decrypt key ${key}:`,
          e.message,
        );
    ...
    return { keys: decryptedKeys, encrypted: true };
  } catch (e) {
    console.error('[ApiConnector] Failed to load API keys:', e.message);
    return { keys: {}, encrypted: true };
  }
}
...
export function saveApiKeys(keys) {
  try {
    ...
    const data = { keys: encryptedKeys, encrypted: true };
    fs.writeFileSync(API_KEYS_FILE, JSON.stringify(data, null, 2));
    return true;
  } catch (e) {
    console.error('[ApiConnector] Failed to save API keys:', e.message);
    return false;
  }
}
...
export function initApiConnector() {
  ...
  if (!fs.existsSync(API_KEYS_FILE)) {
    fs.writeFileSync(
      API_KEYS_FILE,
      JSON.stringify({ keys: {}, encrypted: true }, null, 2),
    );
  }
...
function loadConnectors() {
  try {
    const files = fs
      .readdirSync(API_CONNECTORS_DIR)
      .filter((f) => f.endsWith('.json'));
    for (const file of files) {
      const connector = JSON.parse(
        fs.readFileSync(path.join(API_CONNECTORS_DIR, file), 'utf-8'),
      );
...
export function getApiStats() {
  return {
    ...
    byType: {},
    byAuthType: {},
    activeConnectors: Array.from(apiRegistry.values()).filter(
      (c) => c.status === 'active',
    ).length,
  };
}
