...
export function buildSystemPrompt(options = {}) {
  ...
  // Add coding tool instructions when in project context
  if (context.projectRoot || context.hasFileTools) {
    prompt += getCodingToolInstructions();
  }
...
/**
 * Get instructions for when to use coding/file tools vs inline code
 */
function getCodingToolInstructions() {
  return `

## Coding & File Tools

You have access to file manipulation tools. Use them appropriately:

**Use \`write\` tool when:**
- User asks to "create a file", "save this", "write to file"
- User wants code saved to a specific path
- Creating configuration files, scripts, or any persistent code

**Use \`edit\` tool when:**
- User asks to "modify", "change", "update", "fix" an existing file
- Making targeted changes to existing code
- The oldText must match exactly (including whitespace)

**Use \`read\` tool when:**
- User asks to see a file's contents
- You need to understand existing code before making changes
- Checking file structure or current state

**Use \`list\` tool when:**
- User asks about files in a directory
- You need to explore project structure
- Finding files matching a pattern

**Use \`exec\` tool when:**
- Running build commands, tests, or scripts
- Git operations
- Package management (npm, pip, etc.)
- Any shell command the user requests

**Use \`project_context\` tool when:**
- Starting work on a new project
- User asks about project structure or setup
- You need to understand the tech stack before making suggestions

**Show inline code (don't use tools) when:**
- User asks for an "example" or to "show me"
- Explaining concepts with code snippets
- Quick demonstrations that don't need to be saved
- The user explicitly says "don't save" or "just show"

**Guidelines:**
- Always read a file before editing if you're unsure of its current state
- Use project_context at the start of a coding session to understand the project
- Prefer small, targeted edits over full file rewrites
- Confirm before making destructive changes (overwriting, deleting)`;
}
