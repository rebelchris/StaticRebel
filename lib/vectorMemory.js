...
//
// Inspired by Clawdbot's hybrid memory approach.
...
export async function generateEmbedding(text) {
  ...
  const normalized = truncatedText.toLowerCase().trim();
  ...
  // Create a 384-dimensional sparse vector (MiniLM-Large size) using Float32Array
  const vector = new Float32Array(384);

  // Use hash to determine dimensions
  for (const [word, freq] of wordFreq) {
    const hash = createHash('sha256').update(word).digest();
    const dimIndex = parseInt(hash.slice(0, 4), 16) % 384;
