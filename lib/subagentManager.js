...
/**
 * Initialize the subagent manager with automatic cleanup
 */
export function initSubagentManager() {
  // Start cleanup interval if not already running
  if (!cleanupIntervalId) {
    cleanupIntervalId = setInterval(() => {
      cleanupInactiveSubagents();
    }, CLEANUP_INTERVAL);
...
/**
 * Stop the subagent manager and cleanup interval
 */
export function stopSubagentManager() {
  if (cleanupIntervalId) {
...
/**
 * Cleanup inactive subagents that have exceeded the timeout
 */
function cleanupInactiveSubagents() {
  const now = Date.now();
  const toTerminate = [];
  ...
  for (const [id, subagent] of activeSubagents) {
    if (
      ...
      now - subagent.lastActivity > SUBAGENT_TIMEOUT
    ) {
      toTerminate.push(id);
    }
  }
  ...
  for (const id of toTerminate) {
    console.log(`[SubagentManager] Cleaning up inactive subagent: ${id}`);
    terminateSubagent(id);
  }

  if (toTerminate.length > 0) {
    console.log(
      `[SubagentManager] Cleaned up ${toTerminate.length} inactive subagents`,
    );
  }
}
...
export async function createSubagent(taskType, systemPrompt, options = {}) {
  ...
  if (activeSubagents.size >= MAX_SUBAGENTS) {
    // Try to cleanup idle subagents first
    cleanupInactiveSubagents();

    if (activeSubagents.size >= MAX_SUBAGENTS) {
      throw new Error(
        `Maximum number of subagents (${MAX_SUBAGENTS}) reached. Please terminate some subagents.`,
      );
    }
  }
...
export function cleanupIdleSubagents(maxIdleMs = 3600000) {
  ...
  for (const [id, subagent] of activeSubagents) {
    if (
      subagent.status === 'active' &&
      now - subagent.lastActivity > maxIdleMs
    ) {
      toTerminate.push(id);
    }
  }
