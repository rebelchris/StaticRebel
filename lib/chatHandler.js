...
import { handleSpeakerEvents, skillMeta as speakerEventsMeta } from './skills/speaker-events/index.js';
import {
  TrackerStore,
  QueryEngine,
  parseRecordFromText,
...
import { processInput as processCRMInput, isExplicitCRMQuery, detectContactInteraction } from './skills/crm/index.js';
import { IntentParser, getIntentParser, INTENT_TYPES } from './intent-parser.js';
import { classifyIntent, fallbackClassify, INTENT_TYPES as CLASSIFIER_INTENTS } from './intent/classifier.js';

// NEW: Agentic Intent Parser
import { getAgenticIntentParser, ACTION_TYPES as AGENTIC_TYPES } from './intent/parser.js';
...
// Smart Input Processing (fuzzy matching, multi-stage parsing, momentum)
import {
...
import {
  ...
  extractEntities,
  learn as learnPreference,
  getContext as getMemoryContext,
...
const CONFIG = {
  ...
  USE_SMART_INPUT: process.env.USE_SMART_INPUT !== 'false', // Enable fuzzy matching + momentum
  USE_MOMENTUM: process.env.USE_MOMENTUM !== 'false', // Track conversation patterns
  FUZZY_THRESHOLD: 0.7, // Minimum similarity for typo correction

  // Multi-step Planning
  USE_PLANNER: process.env.USE_PLANNER !== 'false', // Enable multi-step planning for complex tasks
  PLANNER_AUTO_EXECUTE: false, // Auto-execute plans (requires checkpoints)

  // Conversation Memory
  USE_CONVERSATION_MEMORY: process.env.USE_CONVERSATION_MEMORY !== 'false', // Enable conversation memory
  MEMORY_CONTEXT_IN_PROMPTS: true, // Include memory context in LLM prompts

  // Dynamic Tool Discovery
  USE_TOOL_DISCOVERY: process.env.USE_TOOL_DISCOVERY !== 'false', // Enable LLM-powered tool discovery
  TOOL_DISCOVERY_CONFIDENCE: 0.5, // Minimum confidence for tool suggestions
};

// ============================================================================
// Smart Input Processor (singleton)
// ============================================================================

let smartInputProcessor = null;

/**
 * Get or create the smart input processor
 */
function getSmartInputProcessor() {
  if (!smartInputProcessor) {
    smartInputProcessor = createSmartInputProcessor({
      enableMomentum: CONFIG.USE_MOMENTUM,
      enableRedaction: CONFIG.REDACT_SENSITIVE,
      enableSuggestions: true,
      parser: {
        llmThreshold: CONFIG.DEFAULT_CONFIDENCE_THRESHOLD,
      },
    });
  }
...
const INTENT_PATTERNS = {
  ...
  speakerEvents: [
    /speaking (events?|opportunities)/i,
    /conferences?( for| to speak)/i,
    /cfp/i,
    /call for (papers|proposals|speakers)/i,
    /where (can|should) i speak/i,
    /submit (a |my )?(talk|proposal|cfp)/i,
    /tech events?/i,
    /engineering (manager )?conferences?/i,
  ...
  // Coding / File Operations - OpenClaw-style file manipulation
  coding: [
    /create (a |the )?file/i,
    /write (to |a |the )?file/i,
    /save (this |it )?(to |as )(a )?file/i,
    /make (a |the )?file/i,
    /read (the |this )?file/i,
    /show (me )?(the |this )?file/i,
    /open (the |this )?file/i,
    /edit (the |this )?file/i,
    /modify (the |this )?file/i,
    /update (the |this )?file/i,
    /change (the |this )?file/i,
    /fix (the |this )?file/i,
    /list (the )?files/i,
    /what files/i,
    /show (me )?(the )?directory/i,
    /project structure/i,
    /folder structure/i,
    /run (the |this )?command/i,
    /execute (the |this )?command/i,
    /npm (install|run|build|test)/i,
    /yarn (install|add|build|test)/i,
    /pnpm (install|add|build|test)/i,
    /git (status|add|commit|push|pull)/i,
    /create (a |the )?(component|module|class|function|hook)/i,
    /add (a |the )?(component|module|class|function|hook)/i,
    /scaffold/i,
    /generate (a |the )?(component|module|class)/i,
    /refactor (the |this )?/i,
    /implement (the |this |a )?/i,
    /\.js$/i,
    /\.ts$/i,
    /\.jsx$/i,
    /\.tsx$/i,
...
// ============================================================================
// LLM-Based Intent Detection (with pattern fallback)
// ============================================================================

/**
 * Detect intent using LLM classifier with pattern fallback
 * @param {string} text - User input
 * @param {Object} options - Classification options
 * @returns {Promise<Object>} Detected intent with type, entities, confidence, reasoning
 */
async function detectIntentByLLM(text, options = {}) {
  try {
    const result = await classifyIntent(text, options);
    return {
      type: mapClassifierIntent(result.intent),
    ...
    console.error('[ChatHandler] LLM intent classification failed:', error.message);
    return detectIntentByPattern(text);
  }
}

/**
 * Map new classifier intents to existing intent pattern types
 */
function mapClassifierIntent(intent) {
  const mapping = {
    [CLASSIFIER_INTENTS.CODING]: 'coding',
    [CLASSIFIER_INTENTS.SHELL]: 'coding', // Shell maps to coding handler
    [CLASSIFIER_INTENTS.MEMORY]: 'memory',
    [CLASSIFIER_INTENTS.GENERAL]: 'chat',
  };
  return mapping[intent] || 'chat';
}

/**
 * Detect intent using pattern matching (fallback)
 * @param {string} text - User input
...
function detectIntentByPattern(text) {
  ...
  // Use new classifier's fallback for more granular detection
  const fallback = fallbackClassify(text);
  return {
    type: mapClassifierIntent(fallback.intent),
...
/**
 * Detect topic from user input for memory tracking
 * @param {string} text - User input
...
function detectTopic(text) {
  ...
  // Topic patterns
  const topics = [
    { pattern: /\b(code|coding|program|dev|development)\b/i, topic: 'development' },
    { pattern: /\b(schedule|meeting|calendar|event)\b/i, topic: 'scheduling' },
    { pattern: /\b(email|mail|message|send)\b/i, topic: 'communication' },
    { pattern: /\b(track|log|record|water|calories|exercise)\b/i, topic: 'tracking' },
    { pattern: /\b(search|find|look up|research)\b/i, topic: 'research' },
    { pattern: /\b(help|how|what|why|explain)\b/i, topic: 'questions' },
    { pattern: /\b(task|todo|project|plan)\b/i, topic: 'tasks' },
    { pattern: /\b(crm|contact|person|people)\b/i, topic: 'contacts' },
  ];
...
/**
 * Handle CREATE_PROJECT intent - generate and scaffold a new project
 * @param {Object} parseResult - Result from agentic parser
 * @param {Object} context - Execution context
 * @returns {Object} Project creation result
 */
async function handleCreateProject(parseResult, context) {
  const { projectSpec, toolPlan } = parseResult;

  if (!projectSpec) {
    return {
      type: 'create_project',
      action: 'failed',
      content: "I couldn't figure out what kind of project you want to create. Try being more specific, like 'create a todo list in javascript'.",
      confidence: parseResult.confidence
    };
  }
  ...
  try {
    if (CONFIG.DEBUG) {
      console.log(`[ChatHandler] Creating project: ${projectSpec.name} with ${projectSpec.tech?.join(', ')}`);
    }

    const projectPath = await generateProject(projectSpec, context);

    const techList = projectSpec.tech?.join(', ') || '';
    const filesList = projectSpec.files?.length || 0;
    ...
    return {
      ...
      project: projectSpec,
      projectPath,
      content: `✅ Created **${projectSpec.name}** project!

📁 Location: \`${projectPath}\`
🛠️  Tech: ${techList}
📄 Files: ${filesList} generated
...
Would you like me to help with anything else for this project?`,
      confidence: parseResult.confidence
    };
  } catch (error) {
    console.error('[ChatHandler] Project creation failed:', error.message);
    return {
      type: 'create_project',
      action: 'failed',
...
/**
 * Handle WEB_SEARCH intent - search the web for current/realtime info
 * @param {Object} parseResult - Result from agentic parser
 * @param {Object} context - Execution context
 * @returns {Object} Web search result
 */
async function handleWebSearch(parseResult, context) {
  const { webSearchQuery } = parseResult;

  if (!webSearchQuery) {
    return {
      type: 'web_search',
      action: 'failed',
      content: "I couldn't figure out what to search for.",
      confidence: parseResult.confidence
    };
  }
  ...
  try {
    if (CONFIG.DEBUG) {
      console.log(`[ChatHandler] Web search: ${webSearchQuery}`);
    }
    ...
    return {
      ...
      content: result?.content || result?.message || `Here's what I found about "${webSearchQuery}":\n\n${JSON.stringify(result)}`,
      confidence: parseResult.confidence,
      data: result
    };
  } catch (error) {
    console.error('[ChatHandler] Web search failed:', error.message);
    return {
      type: 'web_search',
      action: 'failed',
...
/**
 * Record an action for momentum tracking (call after successful responses)
 * @param {string} action - Action identifier (e.g., "track:water", "command:help")
 * @param {Object} context - Additional context
 */
function recordMomentumAction(action, context = {}) {
  if (CONFIG.USE_MOMENTUM) {
    try {
      const momentum = getMomentumTracker();
      momentum.record(action, context);
    } catch (error) {
      // Silent fail - momentum tracking is non-critical
      if (CONFIG.DEBUG) {
        console.error('[Momentum] Failed to record:', error.message);
      }
    }
  }
}
...
/**
 * Get momentum suggestions for quick actions
 * @returns {Array} List of suggested next actions
 */
export function getMomentumSuggestions() {
  if (!CONFIG.USE_MOMENTUM) return [];
  try {
    const momentum = getMomentumTracker();
    return momentum.getSuggestions();
  } catch {
    return [];
  }
}
...
export async function handleChat(input, options = {}) {
  ...
  const {
    ...
    useSmartInput = CONFIG.USE_SMART_INPUT,
  } = options;
  ...
  // ============================================================================
  // Smart Input Processing (fuzzy matching, typo correction, momentum)
  // ============================================================================
  let smartInput = null;
  ...
  if (useSmartInput) {
    try {
      const processor = getSmartInputProcessor();
      smartInput = await processor.process(input, context);
      
      // If we detected a typo and corrected it, use the corrected input
      if (smartInput.parse?.command?.correctedTo) {
        const correction = smartInput.parse.command;
        correctedInput = input.replace(
          new RegExp(`\\b${correction.originalInput}\\b`, 'i'),
          correction.correctedTo
        );
        if (CONFIG.DEBUG) {
          console.log(`[SmartInput] Typo corrected: "${correction.originalInput}" → "${correction.correctedTo}"`);
        }
      }
      
      // Log momentum suggestions if available
      if (CONFIG.DEBUG && smartInput.suggestions?.length > 0) {
        console.log(`[SmartInput] Momentum suggestions:`, smartInput.suggestions.slice(0, 3).map(s => s.suggestion));
      }
    } catch (error) {
      if (CONFIG.DEBUG) {
        console.error('[SmartInput] Processing failed:', error.message);
      }
      // Continue with original input
    }
  }
  
  // Use corrected input for the rest of processing
  ...
  // ============================================================================
  // CRM Auto-Trigger (runs before everything else for proactive logging)
  // ============================================================================
  let crmAutoLogNote = null;
  try {
    // Check if this is an explicit CRM query - handle it directly
    if (isExplicitCRMQuery(input)) {
      const crmResult = await processCRMInput(input, { source, ...context });
      if (crmResult && crmResult.success) {
        const duration = Date.now() - startTime;
        return {
          success: true,
          content: crmResult.message,
          response: crmResult.message,
          type: crmResult.type,
          data: crmResult.data,
          duration,
          source
    ...
    // Check for contact interaction mentions (proactive auto-logging)
    // This catches things like "I had coffee with Sarah today"
    const detection = detectContactInteraction(input);
    if (detection && detection.confidence >= 0.65) {
      const crmResult = await processCRMInput(input, { source, ...context });
      if (crmResult && crmResult.success && crmResult.type === 'crm_auto_log') {
        // Store the note to append to the response
        crmAutoLogNote = crmResult.message;
        writeDailyMemory(`CRM: ${crmResult.message}`);
        if (CONFIG.DEBUG) {
          console.log(`[ChatHandler] CRM auto-logged: ${crmResult.message}`);
        }
      }
    }
  } catch (error) {
    if (CONFIG.DEBUG) {
      console.log(`[ChatHandler] CRM auto-trigger error: ${error.message}`);
    }
    // Don't fail - CRM is non-critical
  }
  ...
  if (CONFIG.USE_PLANNER) {
    try {
      const complexity = isComplexTask(input);
      if (complexity.isComplex) {
        if (CONFIG.DEBUG) {
          console.log(`[ChatHandler] Complex task detected (${complexity.reason}), creating plan...`);
        }
        ...
        // Format and return the plan for user approval
        const planDisplay = formatPlan(plan);
        const duration = Date.now() - startTime;

        return {
          success: true,
          response: `I've created a plan for this task:\n\n${planDisplay}\n\nWould you like me to execute this plan? (Reply "yes" to proceed, or "modify" to adjust)`,
          content: planDisplay,
          type: 'plan_created',
          action: 'plan',
          planId: plan.id,
      ...
      if (CONFIG.DEBUG) {
        console.log(`[ChatHandler] Planner error: ${error.message}`);
      }
      // Continue to normal routing on planner failure
    }
  }

  // ============================================================================
  // Conversation Memory - Record usage and get context
  // ============================================================================
  let memoryContext = '';
  if (CONFIG.USE_CONVERSATION_MEMORY) {
    try {
      // Record this interaction
      await recordUsage('chat', { topic: detectTopic(input) });

      // Get memory context for LLM prompts
      if (CONFIG.MEMORY_CONTEXT_IN_PROMPTS) {
        memoryContext = await formatContextForPrompt({
          maxSummaries: 3,
          maxPeople: 5,
          maxProjects: 3,
        });
      ...
      if (CONFIG.DEBUG) {
        console.log(`[ChatHandler] Memory error: ${error.message}`);
      }
    }
  }

  // ============================================================================
  // Dynamic Tool Discovery - LLM decides which tools to use
  // ============================================================================
  let discoveredTools = [];
  if (CONFIG.USE_TOOL_DISCOVERY) {
    try {
      discoveredTools = await discoverTools(input, {
        minConfidence: CONFIG.TOOL_DISCOVERY_CONFIDENCE,
        maxSuggestions: 5,
        context: { memoryContext, source },
      });

      if (CONFIG.DEBUG && discoveredTools.length > 0) {
        console.log(`[ChatHandler] Tool Discovery found ${discoveredTools.length} suggestions:`);
        discoveredTools.slice(0, 3).forEach(tool => {
          console.log(`  - ${tool.tool} (${(tool.confidence * 100).toFixed(0)}%): ${tool.reasoning || ''}`);
        });
      }

      // If we have a high-confidence tool match, we can use it to inform routing
      const bestTool = discoveredTools[0];
      if (bestTool && bestTool.confidence >= 0.8) {
        // Handle specific tool categories that can be executed directly
        if (bestTool.capability?.category === 'planning' && bestTool.tool === 'planning:create') {
          // Tool discovery suggests planning - let the planner handle it
          if (CONFIG.DEBUG) {
            console.log(`[ChatHandler] Tool discovery suggests planning tool with ${bestTool.confidence} confidence`);
          }
        }
      }
    } catch (error) {
      if (CONFIG.DEBUG) {
        console.log(`[ChatHandler] Tool discovery error: ${error.message}`);
      }
  ...
  if (useIntelligentRouter) {
    try {
      const result = await routeSimply(input, {
        context: { ...context, memoryContext, discoveredTools },
        source,
      });

      // Append CRM auto-log note if we logged an interaction
      ...
      return {
        ...
        source: source,
        crmAutoLog: crmAutoLogNote ? true : false,
  ...
  // Fall back to LLM-based intent classification (with pattern fallback)
  if (!detectedIntent && CONFIG.USE_PATTERN_MATCHING && !skipPatternMatching) {
    // Use LLM classifier for coding/shell/memory/general intents
    detectedIntent = await detectIntentByLLM(input, { context });
    debugLog(`LLM classifier: ${detectedIntent.intent || detectedIntent.type} (${detectedIntent.confidence.toFixed(2)}) - ${detectedIntent.reasoning || detectedIntent.method}`);
  ...
  if (!skipLLMClassification) {
    try {
      // First try agentic parser for better understanding
      const agenticParser = await getAgenticIntentParser();
      const agenticResult = await agenticParser.parse(input, context);

      if (CONFIG.DEBUG) {
        console.log(`[ChatHandler] Agentic intent: ${agenticResult.action} (${agenticResult.confidence})`);
      }

      // Handle agentic results for CREATE_PROJECT and WEB_SEARCH
      if (agenticResult.action === AGENTIC_TYPES.CREATE_PROJECT) {
        const duration = Date.now() - startTime;
        const projectResult = await handleCreateProject(agenticResult, context);
        return {
          ...projectResult,
          duration,
          source,
      ...
      if (agenticResult.action === AGENTIC_TYPES.WEB_SEARCH) {
        const duration = Date.now() - startTime;
        const webResult = await handleWebSearch(agenticResult, context);
        return {
          ...webResult,
          duration,
          source,
      ...
      // Fall back to enhanced parser for tracking
...
async function handleBuiltInIntent(intentType, input, context) {
  switch (intentType) {
    ...
    case 'coding':
      return await handleCodingIntent(input, context);

    case 'speakerEvents':
      return await handleSpeakerEventsIntent(input, context);
...
/**
 * Handle speaker events / conferences intent
 * @private
 */
async function handleSpeakerEventsIntent(input, context) {
  try {
    const result = await handleSpeakerEvents(input, context);
    return result.text;
  } catch (error) {
    console.error('[ChatHandler] Speaker events error:', error);
    return "Sorry, I couldn't fetch conference information right now. Try again in a moment.";
  }
}

/**
 * Handle coding/file operation intents
 * Determines whether to use file tools or inline code based on user request
 * @private
 */
async function handleCodingIntent(input, context) {
  const lower = input.toLowerCase();
  
  try {
    // Lazy import the tool system
    const { getToolSystem } = await import('./tools/index.js');
    const toolSystem = getToolSystem();
    await toolSystem.initialize();
    
    // Determine the appropriate action based on input
    
    // File creation patterns
    if (/create (a |the )?file|write (to |a |the )?file|save.*(to |as ).*(file)?/i.test(lower)) {
      // Extract file path if mentioned
      const pathMatch = input.match(/(?:to|as|file|named?)\s+[`"']?([^\s`"']+\.\w+)[`"']?/i);
      
      if (pathMatch) {
        const filePath = pathMatch[1];
        return `I'll create/write to \`${filePath}\`. Please provide the content you want to write, or I can help you generate it.

**Available tools:**
- \`write\` - Create or overwrite the file
- \`edit\` - Modify existing content

What would you like to put in \`${filePath}\`?`;
      }
      
      return `I can help you create or write to a file. Please specify:
1. The file path (e.g., \`src/utils/helper.js\`)
2. The content you want to write

Or describe what you need, and I'll help generate the code!`;
    }
    
    // File reading patterns
    if (/read (the |this )?file|show (me )?(the |this )?file|open (the |this )?file/i.test(lower)) {
      const pathMatch = input.match(/(?:read|show|open)\s+(?:me\s+)?(?:the\s+)?(?:file\s+)?[`"']?([^\s`"']+\.\w+)[`"']?/i);
      
      if (pathMatch) {
        const filePath = pathMatch[1];
        const result = await toolSystem.executeTool('read', { path: filePath }, context);
        
        if (result.success) {
          const data = result.result;
          return `📄 **${data.path}** (${data.totalLines} lines, ${data.size} bytes)

\`\`\`
${data.content}
\`\`\`

${data.truncated ? '⚠️ Output truncated. Use offset/limit for more.' : ''}`;
        } else {
          return `❌ Failed to read file: ${result.error}`;
        }
      }
      
      return `Which file would you like me to read? Please provide the path.`;
    }
    
    // File editing patterns
    if (/edit (the |this )?file|modify (the |this )?file|update (the |this )?file|change (the |this )?file|fix (the |this )?file/i.test(lower)) {
      const pathMatch = input.match(/(?:edit|modify|update|change|fix)\s+(?:the\s+)?(?:file\s+)?[`"']?([^\s`"']+\.\w+)[`"']?/i);
      
      if (pathMatch) {
        const filePath = pathMatch[1];
        return `I'll help you edit \`${filePath}\`. 

The \`edit\` tool requires:
1. **oldText** - The exact text to find (must match perfectly)
2. **newText** - What to replace it with

Please describe the change you want to make, or provide the old and new text.`;
      }
      
      return `Which file would you like to edit? Please provide the path and describe the changes.`;
    }
    
    // Directory listing patterns
    if (/list (the )?files|what files|show (me )?(the )?directory|project structure|folder structure/i.test(lower)) {
      const pathMatch = input.match(/(?:in|at|from)\s+[`"']?([^\s`"']+)[`"']?/i);
      const dirPath = pathMatch ? pathMatch[1] : '.';
      
      const result = await toolSystem.executeTool('list', { path: dirPath }, context);
      ...
      if (result.success) {
        ...
        return `📂 **${data.path}** (${data.count} items)

\`\`\`
${listing}
\`\`\``;
      } else {
        return `❌ Failed to list directory: ${result.error}`;
      }
    }
    
    // Command execution patterns
    if (/run (the |this )?command|execute (the |this )?command|npm |yarn |pnpm |git /i.test(lower)) {
      const cmdMatch = input.match(/(?:run|execute)\s+(?:the\s+)?(?:command\s+)?[`"']?(.+?)[`"']?\s*$/i) ||
                       input.match(/(npm\s+\S+.*|yarn\s+\S+.*|pnpm\s+\S+.*|git\s+\S+.*)/i);
      
      if (cmdMatch) {
        const command = cmdMatch[1];
        const result = await toolSystem.executeTool('exec', { command, timeout: 30000 }, context);
...
Exit code: ${data.exitCode}${data.timedOut ? ' (timed out)' : ''}
...
${output || 'No output.'}`;
    ...
    if (/project structure|what.*(project|tech|stack|framework)/i.test(lower)) {
      const result = await toolSystem.executeTool('project_context', {}, context);
      
      if (result.success) {
        const data = result.result;
        let response = `🔍 **Project Analysis**

**Type:** ${data.type} (${data.language})
**Frameworks:** ${data.frameworks.length > 0 ? data.frameworks.join(', ') : 'None detected'}
**Entry Points:** ${data.entryPoints.length > 0 ? data.entryPoints.join(', ') : 'None found'}

**Config Files:** ${data.configFiles.slice(0, 10).join(', ')}${data.configFiles.length > 10 ? '...' : ''}
...
**Package:** ${data.packageInfo.name} v${data.packageInfo.version}
**Scripts:** ${data.packageInfo.scripts.slice(0, 5).join(', ')}${data.packageInfo.scripts.length > 5 ? '...' : ''}
**Dependencies:** ${data.packageInfo.dependencyCount} prod, ${data.packageInfo.devDependencyCount} dev`;
        }
        
        return response;
      } else {
        return `❌ Failed to analyze project: ${result.error}`;
      }
    }
    
    // Generic coding help - show available tools
    return `🛠️ **Coding Tools Available:**
...
What would you like to do?`;
    ...
    console.error('[ChatHandler] Coding intent error:', error);
    return `❌ Error with coding tools: ${error.message}`;
  }
}
...
// Dynamic Tool Discovery exports
export {
  discoverTools,
  getTools,
  getToolCapabilities,
