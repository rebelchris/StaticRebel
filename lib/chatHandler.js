...
import { applyPersonalityFilter, getPersonalitySystemPrompt } from './personality/index.js';
...
// NEW: Hybrid Router (LLM extraction + deterministic execution)
import { routeHybrid } from './hybridRouter.js';
...
export async function handleChat(input, options = {}) {
  ...
  // ============================================================================
  // NEW: Hybrid Router Mode (LLM extraction + deterministic execution)
  // ============================================================================
  if (useIntelligentRouter) {
    try {
      const result = await routeHybrid(input, {
        context,
        source,
      });
      ...
      console.error('[ChatHandler] Hybrid router failed, falling back to legacy:', error.message);
...
async function determineAndExecuteAction(
    ...
    if (matchingAction) {
      ...
      if (result.success) {
        return {
          ...
          content: applyPersonalityFilter(result.result),
  ...
  try {
    ...
    console.error('[ChatHandler] Chat fallback failed:', e.message, '- Endpoint: /api/chat');
  }

  // Handle empty responses (e.g., Ollama not running)
  if (!chatContent.trim()) {
    return {
      type: 'chat',
      content: "Sorry, I can't connect to Ollama right now. Make sure Ollama is running with 'ollama serve'.",
      confidence: 1.0,
    };
  }
