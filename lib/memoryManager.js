...
const MEMORY_DIR = path.join(os.homedir(), '.ollama-assistant', 'memory');
...
export function initMemory() {
  ...
  if (!fs.existsSync(LONG_TERM_FILE)) {
    fs.writeFileSync(LONG_TERM_FILE, `# Long-Term Memory
...
`);
  ...
  if (!fs.existsSync(HEARTBEAT_STATE_FILE)) {
    fs.writeFileSync(HEARTBEAT_STATE_FILE, JSON.stringify({
      lastChecks: {
        email: null,
        calendar: null,
        mentions: null,
...
export function curateMemory() {
  ...
  const summary = recent.map(r => `## ${r.date}\n${r.content}`).join('\n\n---\n\n');
...
export function loadSessionMemory() {
  ...
  return {
    ...
    yesterday: readDailyMemory(new Date(Date.now() - 86400000).toISOString().split('T')[0]),
